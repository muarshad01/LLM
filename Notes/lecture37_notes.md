...
[Music]
0:05
hello everyone and uh welcome to this
0:07
lecture in the build large language
0:09
models from scratch Series today we are
0:13
going to look at a topic which is called
0:15
as instruction fine
0:17
tuning in the previous set of lectures
0:20
we have covered this much so far in
0:22
building a large language model
0:24
initially in the first set of lectures
0:27
we saw how to prepare data we learned
0:30
everything about the attention mechanism
0:32
and we learned about the llm
0:34
architecture this was the stage one the
0:37
stage two was the pre-training process
0:39
where we learned about the pre-training
0:41
loop model evaluation and then we
0:44
learned how to load pre-rain weights
0:46
given by open AI gpt2 to construct our
0:49
foundation model and this Foundation
0:51
model is also called as pre-trained
0:56
model and then we started looking at
0:58
fine tuning so until this point in our
1:01
lecture series we have looked at one
1:03
type of f fine tuning which is called as
1:06
classification based fine tuning in
1:08
which we were given a set of emails and
1:10
then the task was to fine tune our
1:13
pre-train model so that it can classify
1:16
any new email as whether it's a Spam or
1:19
not a Spam and we completed this
1:21
Hands-On project fully from scratch and
1:24
now what we are going to do is that we
1:26
are going to start looking at another
1:28
category of fine tuning which is is much
1:30
more common and that's called as
1:32
instruction based fine
1:37
tuning instruction based fine tuning in
1:40
these series of lectures which will be
1:42
five to six lectures we'll build our own
1:46
personal assistant from scratch so we
1:48
have already we already have a
1:51
pre-trained model but the pre-trained
1:53
model needs to be improved and it needs
1:55
to be fine tuned further so that it can
1:58
serve as our own personal assistant
2:00
so let's get started with this chapter
2:02
in which we will fine tune the
2:04
pre-trained llm to follow
2:06
instructions first we need to understand
2:09
why do we need fine tuning right so the
2:11
main reason why we need fine tuning is
2:14
that pre-trained large language large
2:16
language models uh like the one which we
2:18
trained in this series so far are quite
2:20
good at text completions but they really
2:24
struggle with following instructions so
2:26
what are these instructions let's say if
2:28
you use the pre-trained M and if you ask
2:31
it to to fix the grammar in the text it
2:33
will not be able to do this if you ask
2:36
the llm convert the text from an active
2:38
voice to a passive voice it will not be
2:40
able to do this uh so similar
2:44
instructions are difficult to follow for
2:47
a pre-trained large language model
2:49
without fine tuning without fine tuning
2:52
an llm can generate new tokens it can
2:54
generate new sentences fully but it
2:57
cannot follow instructions
3:01
and we want our large language model to
3:03
follow instructions if we want to build
3:04
a personal assistant right if we tell
3:07
the personal assistant that hey look at
3:09
my sentence correct the grammar or hey
3:11
look at my sentence remove all the does
3:13
and the O in the sentence or hey look at
3:16
the sentence and make it very short make
3:18
it concise uh and make the tone very
3:21
professional it won't be able to
3:23
understand these instructions without
3:24
fine tuning and that's why we have to
3:27
learn about fine tuning I went to chat
Instruction fine-tuning examples
3:29
GPT and I asked Chad GPT give two
3:31
examples of fine tuning in real life and
3:34
here's what Chad GPT came up with so
3:37
imagine first of all you are in an
3:39
e-commerce company who wants to build a
3:41
customer support
3:42
chatbot so suppose an e-commerce company
3:45
like Amazon uh wants to or Walmart let's
3:49
say wants to develop or deploy a
3:51
customer support chatbot that helps
3:54
users with order status returns and
3:56
product recommendations so they want to
3:58
build an aias bot where users can go and
4:01
ask the chatbot some questions such as
4:03
what's the refund policy or if I want to
4:06
return an order what's the policy with
4:08
respect to that now if the company
4:11
starts with only a pre-train model uh it
4:14
will be very difficult for the model to
4:16
understand specific customer queries
4:18
which is based on the company's policies
4:20
which is based on the company's products
4:22
policies and
4:23
services so the model won't be able to
4:26
answer the questions which are given by
4:28
the user since since it is not
4:30
instruction finetuned yet and as a
4:33
result we need to finetune the model to
4:36
understand specific customer queries
4:37
about the company's own products
4:39
policies and
4:40
services so the reason this is necessary
4:43
is that the base language model which is
4:44
only pre-trained may not have knowledge
4:47
specific to the company's policies or
4:49
products because many of times this
4:50
information is private instruction find
4:53
tuning allows the company to provide
4:55
domain specific instructions such as if
4:58
a user asks about return policy provide
5:00
steps to in initiate a return or if a
5:03
