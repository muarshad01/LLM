...
0:07
lecture in the built large language
0:09
models from scratch Series in these set
0:12
of lectures we have been learning about
0:15
the llm architecture in a lot of detail
0:18
if you want to learn about the llm
0:20
architecture it is very important that
0:23
you understand all of these building
0:25
blocks from bottom to top in the
0:28
previous lecture we covered about the G
0:30
DPT backbone which is essentially a
0:32
dummy code with the bird's eye view
0:35
where you see all the different
0:36
components together without coding any
0:39
of them and since the last lecture we
0:41
have started looking at each individual
0:43
component and coding them out in python
0:46
in the previous lecture we looked at
0:48
this building block called layer
0:50
normalization and today's lecture is
0:53
going to be completely focused on the
0:55
second building block which is the JLo
0:58
activation function along along with
1:00
this feed forward neural network so
1:02
today actually we are going to cover two
1:04
of these blocks which have been shown
1:06
over here the J activation and the feed
1:09
forward neural network in the next
1:12
lecture we'll also be looking at
1:13
shortcut connections and then all of
1:16
these building blocks will come together
1:18
to form the Transformer block which is
1:20
the Beating Heart of the GPT
1:22
architecture so let's continue along our
1:25
journey and make incremental progress
1:27
towards understanding the Transformer
1:29
block as I mentioned the goal the
1:31
objective of today's lecture is to
1:34
understand the J activation along with
1:36
the feed forward neural network so let's
1:38
get started with today's lecture to give
1:40
you an overview of what the Transformer
1:43
block actually looks like here's the
1:46
architecture of the Transformer block
1:48
zooming into this Transformer so the
1:51
Transformer block consists of many
1:53
layers which are stacked together and
1:55
that's why we have to learn about each
1:57
of these layers separately the first is
2:00
the layer normalization layer the input
2:03
then is normalized and it's passed to
2:04
the multi-ad attention we have a Dropout
2:07
layer after that this plus sign
2:09
indicates that there is a shortcut
2:11
connection then we have another layer
2:13
normalization layer we learned about
2:15
layer normalization in the previous
2:17
lecture the output of the second layer
2:19
normalization passes into a feed forward
2:21
neural network which is what we are
2:23
going to learn about in today's lecture
2:25
and when you put a microscope to this
2:27
feed forward neural network and try to
2:29
really understand understand what it
2:30
consists of you'll see that it consists
2:33
of linear layers which are common in any
2:36
neural network but it consists of this
2:38
jalu or guu
2:40
activation which is you might be hearing
2:43
about this for the first time even if
2:45
you have use deep learning Frameworks
2:47
people generally use reu T sigmoid Etc
2:51
but this jalu activation or G activation
2:53
is new to most of the people everything
2:56
else in the speed forward neural network
2:58
is same as traditional neural networks
3:01
but this jalu activation is something
3:02
which is a bit
3:04
different so we'll be covering this part
3:07
also in a lot of detail so I just wanted
3:09
to show you this so that you you have
3:11
some context about what why we are
3:14
learning this lecture and after you
3:16
understand the feed forward neural
3:17
network and guu after today's lecture
3:20
you'll be able to essentially understand
3:22
most of these components over here
3:23
except for the shortcut connection which
3:25
we'll look at in the next lecture okay
3:28
so let's begin so the goal of this
3:30
lecture is to implement a small neural
3:33
network subm module that is part of the
3:36
llm Transformer block and when I say
3:38
neural network subm module I'm
3:40
especially referring to this feed
3:42
forward block which has been mentioned
3:44
uh here this this feed forward block I'm
3:47
referring to this block we want to write
3:50
a code to uh uh to integrate this block
3:55
this feed forward neural network
3:57
block okay so now let us first learn
GELU activation mathematics
4:00
about the guu or the jalu activation
4:02
function I'm going to call it jalu for
4:04
the rest of this lecture uh but the way
4:07
you call it does not really matter as
4:09
long as you understand what the function
4:12
is what does it represent and why don't
4:15
we use the common activation functions
4:16
like Ru so two activation functions
4:20
which are very commonly implemented in
4:21
large language models are the jalu and
4:24
the Swig Loop in today's lecture we are
4:26
going to specifically look at the
4:28
JLo so before we look at that first
4:31
let's understand about the reu
4:32
activation function this is how the reu
4:35
activation function actually looks like
4:37
for X which is greater than zero The Rao
4:39
activation function Returns the same
4:41
value as the input but if x is less than
4:44
zero The Rao activation function returns
4:46
zero so if you plot The Rao activation
4:49
function as a function of X you'll see
4:51
that the curve looks something like this
4:53
it's flat for negative values of X and
4:55
then it's linear already you can start
4:58
seeing some problems with this right
...
5:00
it's not differentiable at x equal to0
5:03
there is a sharp jump over here and for
5:05
all the values of X less than 0 this is
5:08
equal to zero so if you integrate this
5:11
in neural networks it often leads to a
5:13
dead neuron problem which means that if
5:16
the output from one layer is negative
5:19
and if Ru activation function is applied
5:21
to it the output becomes zero and then
5:23
it stays zero because uh because we
5:27
cannot do any learning of after that so
5:31
the neurons which are associated with
5:32
that particular output they don't
5:35
contribute anything to the learning
5:36
process once the output of the neuron
5:39
becomes negative and that's called as
5:40
the dead neuron problem so learning
5:42
essentially stagnates of course Ru has a
5:45
huge number of other advantages this
5:47
nonlinearity which is introduced over
5:48
here makes neural networks expressive it
5:51
gives the power to neural networks but
5:54
the reason we we are looking at the
5:55
disadvantages of reu is that
5:58
understanding the disadvantages of Ru
6:00
will open an opportunity for us to learn
6:02
about the jalu activation function and
6:04
why it is used in large language models
6:07
